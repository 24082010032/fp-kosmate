  ...
  <div class="mx-auto max-w-4xl">
    ...
    <form action="{{ route('pemilik.kamar.update', $kamar->id) }}" method="POST" class="space-y-6 rounded-3xl bg-white border border-white/80 p-8 shadow-lg shadow-slate-200/80">
      ...
      <div>
        ...
        <input name="name" type="text" value="{{ $kamar->nama }}" class="mt-2 w-full rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-pink-400" />
      </div>
      <div>
        <label class="font-semibold text-slate-700">Harga</label>
        <input name="price" type="text" value="{{ $kamar->harga }}" class="mt-2 w-full rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-400" />
      </div>
      <div>
      ...
      <div>
        ...
        <textarea name="description" rows="4" class="mt-2 w-full rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-pink-400">{{ $kamar->fasilitas }}</textarea>
      </div>
