    ...
    <header class="mb-10 rounded-4xl border border-white/80 bg-white/80 p-8 shadow-2xl shadow-pink-200/30">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex flex-wrap gap-3">
          <a href="/pemilik/dashboard" class="rounded-3xl bg-amber-100 px-5 py-3 font-semibold text-amber-900 shadow-lg shadow-amber-200/50">Dashboard Pemilik</a>
      ...
      @foreach($kamar as $item)
        <article class="rounded-3xl border border-white/80 bg-white p-6 shadow-lg shadow-slate-200/80">
          <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p class="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">{{ $item->tipe_kamar }}</p>
              <h2 class="mt-2 text-2xl font-bold text-slate-900">Rp{{ number_format($item->harga, 0, ',', '.') }}</h2>
